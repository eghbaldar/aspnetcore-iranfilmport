...
namespace IranFilmPort.Application.Services.UserProjects.Queries.GetUserProjectForUser
{
    public class RequestGetUserProjectForUserServiceDto
    {
        public Guid? Id { get; set; }
        public Guid? UserId { get; set; }
    }
    public class GetUserProjectForUserServiceDto
    {
        public Guid Id { get; set; }
    ...
    public interface IGetUserProjectForUserService
    {
        GetUserProjectForUserServiceDto Execute(RequestGetUserProjectForUserServiceDto req);
    }
    public class GetUserProjectForUserService : IGetUserProjectForUserService
    {
        private readonly IDataBaseContext _context;
        public GetUserProjectForUserService(IDataBaseContext context)
        {
            _context = context;
        }
        public GetUserProjectForUserServiceDto Execute(RequestGetUserProjectForUserServiceDto req)
        {
            if (req == null || req.Id == Guid.Empty || req.Id == Guid.Empty) return null;
            var result = _context.UserProjects
                .Where(x => x.Id == req.Id && x.UserId == req.UserId)
                .Select(x => new GetUserProjectForUserServiceDto
                {
                    Id = x.Id,
