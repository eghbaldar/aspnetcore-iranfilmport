...
{
    /// <summary>
    /// دقت کن که این اتریبوت برای آی اکشن ریزالت هایی است که میخواهیم حتما کاربر لاگین کرده از آن استفاده کند
    /// </summary>
    [AttributeUsage(AttributeTargets.Method | AttributeTargets.Class)]
    public class KingAuthorizeAttribute : Attribute, Microsoft.AspNetCore.Mvc.Filters.IAuthorizationFilter
    {
        private readonly UserRole[] _roles;
        public KingAuthorizeAttribute(params UserRole[] roles)
        {
            _roles = roles;
        }
        ...
        {
            ///////////////////////////////////////////////////////////////////////
            // check come roles from IActionResult
            // it means this attribute doesn't work without roles!
            if (_roles.Length == 0)
            {
            ...
            var users = serviceProvider.GetRequiredService<UsersFacadePattern>();
            ...
            // redirect unauthroized user to the page
            // user == null => unauthroized user
            // user != null => auhtorized user
            if (user == null) context.Result = new RedirectToActionResult(TokenStatics.DestinationActionAfterLogout, TokenStatics.DestinationControllerAfterLogout, null);            
        }
        ...
        private UserTokenDto CheckAccessLogic(HttpContext httpContext, UserRefreshTokenService refreshTokenService, params UserRole[] roles)
        {
            ...
            {
                /*
                GetUserId => NameIdentifier
                GetUserHeadshot => UserData
                GetUserFullname => Name
                GetUserUsername => Actor
                GetUserEmail => Email
                 */
